...
// 解析CKEditor<a>標籤
    ...
    echo "<script>
    alert('文章新增成功！');
    window.location = '?page=article_index';
    </script>";
    ...
    echo "<script>
    window.parent.CKEDITOR.tools.callFunction($funcNum, '".addslashes($fileUrl)."', '');
    </script>";
    } else {
    // 給 JSON 模式（貼上/拖曳上傳）
    header('Content-Type: application/json; charset=utf-8');
    ...
    // 使用flags避免DOM自動補html, body
    $doc->loadHTML('
    <?xml encoding="utf-8" ?>' . $content, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
    libxml_clear_errors();
